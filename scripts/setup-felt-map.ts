/**
 * Setup script: Creates a Felt map and populates it with all 97 Ikon Pass resorts.
 *
 * Usage:
 *   FELT_API_TOKEN=your_token npx tsx scripts/setup-felt-map.ts
 *
 * Prerequisites:
 *   - A Felt account with API access (https://felt.com)
 *   - An API token from Felt (Settings → Integrations → API Tokens)
 */
...
// Import resort data inline (same as src/data/resorts.ts but standalone for the script)
...
async function feltRequest(
  ...
  if (!token) {
    throw new Error(
      "FELT_API_TOKEN environment variable is required.\n" +
        "Get one from: Felt → Settings → Integrations → API Tokens"
    );
  }

  const url = `${FELT_API}${path}`;
...
async function main() {
  console.log("\n🗺️  Ikon Pass Map Setup\n");
  console.log(`Found ${resorts.length} resorts to add.\n`);

  // Step 1: Create the map
  console.log("Creating Felt map...");
  const createRes = await feltRequest("/maps", {
    method: "POST",
    body: JSON.stringify({
      ...
      description:
        "All 97 Ikon Pass ski resorts for the 2025/26 season, colour-coded by region.",
    }),
  });

  const map = await createRes.json();
  const mapId: string = map.id;
  const mapUrl: string = map.url;

  console.log(`✓ Map created: ${mapUrl}`);
  console.log(`  Map ID: ${mapId}\n`);

  // Step 2: Add resort elements in batches (to stay under 1MB limit)
  const BATCH_SIZE = 50;
  for (let i = 0; i < resorts.length; i += BATCH_SIZE) {
    ...
    console.log(
      `Adding resorts ${i + 1}–${Math.min(i + BATCH_SIZE, resorts.length)}...`
    );
  ...
  console.log(`✓ All ${resorts.length} resorts added.\n`);
  ...
  console.log(`   NEXT_PUBLIC_FELT_MAP_ID=${mapId}\n`);
  console.log("2. View your map at:");
  console.log(`   ${mapUrl}\n`);
  console.log(
    "3. (Optional) Customize the map styling in the Felt UI at the URL above.\n"
  );
  console.log(
    "4. For Vercel deployment, add the same env var in your Vercel project settings.\n"
  );
}
